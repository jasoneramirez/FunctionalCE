...
def modelo_opt(leaves, values, restricciones_right, restricciones_left, x, y, y_pred, model,datos_arbol,path_sol):
    ...
    modelo_opt.times = RangeSet( 0,modelo_opt.N-1 ) #conjunto de tiempos  
    ...
    modelo_opt.M1=Param(modelo_opt.times,within=PositiveReals) #m1 grande
    modelo_opt.M2=Param(modelo_opt.times,within=PositiveReals) #m2 grande #Positive reals
    ...
    modelo_opt.vec=Param (modelo_opt.vecinos,modelo_opt.times) #el vecino de la otra clase




    #path para calcular la distancia
    #modelo_opt.index_path=RangeSet(0,modelo_opt.N-1)
    #modelo_opt.path=Param(modelo_opt.index_path, within=Any)
    modelo_opt.path_index=Set(dimen=2, initialize=path_sol)
    ...
    #distancia l2 mas derivadas l2(f,g)+l2(f',g')
    #def obj_rule(modelo_opt):
    #    return 0.5*sum((modelo_opt.x0[n]-modelo_opt.x[n])**2 for n in modelo_opt.t)+0.5*sum((modelo_opt.x0_d[n]-modelo_opt.x_d[n])**2 for n in modelo_opt.t_d)
    #modelo_opt.obj =Objective(rule=obj_rule)

    #distancia derivadas l2(f',g')
    #def obj_rule(modelo_opt):
    #    return sum((modelo_opt.x0_d[n]-modelo_opt.x_d[n])**2 for n in modelo_opt.t_d)
    #modelo_opt.obj =Objective(rule=obj_rule)

    #distancia derivadas l2(f',g')+l2(f'',g'')
    #def obj_rule(modelo_opt):
    #    return 0.5*sum((modelo_opt.x0_d[n]-modelo_opt.x_d[n])**2 for n in modelo_opt.t_d)+0.5*sum((modelo_opt.x0_d2[n]-modelo_opt.x_d2[n])**2 for n in modelo_opt.t_d2)
    #modelo_opt.obj =Objective(rule=obj_rule)

    #distancia int(f'-g')+int(f''-g'')
    #def obj_rule(modelo_opt):
    #    return 0.5*sum(((modelo_opt.x0_d[n]-modelo_opt.x_d[n])**2+(modelo_opt.x0_d[n+1]-modelo_opt.x_d[n+1])**2)/2 for n in modelo_opt.t_d2)+0.5*sum(((modelo_opt.x0_d2[n]-modelo_opt.x_d2[n])**2+(modelo_opt.x0_d2[n+1]-modelo_opt.x_d2[n+1])**2)/2 for n in modelo_opt.t_d3)
    #modelo_opt.obj =Objective(rule=obj_rule)

    #distancia int(f'-g')
    #def obj_rule(modelo_opt):
    #    return sum((modelo_opt.dif_d1[n]+modelo_opt.dif_d1[n+1])/2 for n in modelo_opt.t_d2)
    #modelo_opt.obj =Objective(rule=obj_rule)
    ...
    #def derivada(modelo_opt,n):
    #    return modelo_opt.x_d[n]==modelo_opt.x[n+1]-modelo_opt.x[n]
    #modelo_opt.deriv=Constraint(modelo_opt.t_d,rule=derivada)

    #def derivada2(modelo_opt,n):
    #    return modelo_opt.x_d2[n]==modelo_opt.x_d[n+1]-modelo_opt.x_d[n]
    #modelo_opt.deriv2=Constraint(modelo_opt.t_d2,rule=derivada2)

    #def dif_d1(modelo_opt,n):
    #    return modelo_opt.dif_d1[n]==(modelo_opt.x0_d[n]-modelo_opt.x_d[n])**2
    #modelo_opt.dif1=Constraint(modelo_opt.t_d,rule=dif_d1)

    #def dif_d2(modelo_opt,n):
    #    return modelo_opt.dif_d2[n]==(modelo_opt.x0_d2[n]-modelo_opt.x_d2[n])**2
    #modelo_opt.dif2=Constraint(modelo_opt.t_d2,rule=dif_d2)
